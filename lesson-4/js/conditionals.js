...
function setWeather() {
	...
	// console.log(choice, temperature);
...
function setBackground(temperature) {
	// change screen color based on temperature
	if (temperature >= 15) {
		page.style.backgroundColor = 'orange'; // background-color
	} else {
...
function getWindSpeed() {
	...
	// STEP 5b: Build out four cases, followed by a default
		...
		case 0:
			// 0 km/h
			windComment.textContent = speed + 'km/h - It\'s calm as can be.';
			break;
		default:
			windComment.textContent = speed + 'km/h - I don\'t have a comment for that wind speed.';
...
coffeeBreak ? breakMessage.textContent = "Time for a coffee break!" : breakMessage.textContent = "No break for you, keep working!";
